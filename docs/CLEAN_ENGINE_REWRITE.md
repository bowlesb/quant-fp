...
| **trust- AND capture-load-bearing — KEEP** | **~1,962** — `compare.py` 174 / `validation_sweep.py` 809 / `validate.py` 581 / `crypto_validation_sweep` 263 / `parity.py` 135 | **HARD KEEP** — `compare.py` is imported by BOTH the live trust certifier (`within_day_parity`/`crypto_within_day` grade via `cell_verdict`/`dist_score`) AND the live CAPTURE path (`capture.py:26` + `materialize.py:23` import `compare.runnable`) — it is live-capture-load-bearing, not just trust. `validation_sweep` → `trust_random_check.py:37` (weekly trust re-verify); `validate` → `crypto_validation_sweep.py:44` (crypto trust grant). `parity.py` is the live `make parity` T+1 settled-day cornerstone — it catches input/settled-reality divergence (live provisional bars vs settled revised bars) the backfill==replay invariant does NOT address; only its compute-form-drift half is obsoleted. These delete ONLY after the trust certifier is re-pointed off `compare.cell_verdict` AND `compare.runnable` is dropped from capture/materialize — a separate post-greenlight step, NOT tonight. NOT byte-parity-between-forms. |
...
- **the live TRUST / CERT system — HARD KEEP, NOT byte-parity:** `compare.py`, `validate.py`,
  ...
  never an overnight delete. (Import-verified: the live capture path itself imports `compare.runnable` —
`capture.py:26` + `materialize.py:23` — so `compare.py` is live-capture-load-bearing, not just trust.)
