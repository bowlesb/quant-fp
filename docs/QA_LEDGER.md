...
| P1 | backfill-split-jump | **NEW class (task #17): mixed split-adjustment states in backfill** — a split landing mid-backfill leaves pre-split months on the old adjustment basis, deflating prices (KLAC = confirmed 10× artifact). A >3× day-jump sweep flagged **11 names**; KLAC confirmed artifact, the other 10 are likely real moves/reverse-splits (prod verifying each). This is a price-integrity bug that reaches the PANEL (momentum features), not just the parity overlap. | OPEN — new automated invariant `no_extreme_backfill_jump` (>3× unexplained backfill day-over-day close jump, cross-checked vs a corporate-action allowlist so real reverse-splits don't false-positive) added to the suite; runs in the post-rebuild battery. We have NO corporate-action data source yet → allowlist is curated (tests/fixtures/known_corporate_actions.txt). Prod owns the backfill fix. |
