...
| P0 | etf-contamination | **🔴 REGRESSED 2026-06-15 — WORSE THAN THE ORIGINAL. universe_membership for 2026-06-15 = 11,336 in_universe members, 34.3% (3,889) on the frozen fund denylist** (was a clean ~1000 / 0 funds on 6/12). The equities-only filter (`is_etf_like` in `select_universe`, fix 814e548) is NOT applied to this rebuild — the universe ballooned 1000→11,336 and re-admitted the entire fund class (SOXL/SQQQ/TQQQ/UPRO/SPXU/TSLL/GLD/SLV/XLF/XLE + thousands more; AAPB/AAPD/AAPU = AAPL leveraged). Both composition invariants FAIL today (`universe_is_equities_only` + `universe_no_known_funds`, 3,842+ denylist hits). **DOWNSTREAM IMPACT confirmed in cross_sectional_rank live store (2026-06-15 audit):** that group doesn't even use the universe pin — it ranks over RAW minute_agg (whoever printed), 9,913 distinct symbols/day, nsym up to 4,809/min, **32.8% (3,256) of the cross-section are known funds.** So EVERY single-name equity's percentile rank today is computed against a one-third-funds cross-section incl. leveraged/inverse products = the exact lesson reproduced. ORIGINAL ROOT CAUSE (still valid): `is_etf_like` wired into executor (main.py:112) but skipped in `select_universe`. NEW question: was the filter reverted, or did a re-scale/rebuild path bypass it? Routes to prod (owns universe build) — URGENT. | **🔴 REOPENED P0 (2026-06-15).** Was PARTIAL/green on 6/12 (0/1000); regressed to 34.3% funds / 11,336 members today. The automated invariants CAUGHT it (fail-loud worked) but it is LIVE and polluting every cross-sectional feature now. cross_sectional_rank is BLOCKED-ON-SYSTEMIC until (a) prod re-applies the equities-only filter to the universe rebuild AND (b) the group is wired to the pinned universe (it currently ranks over raw minute_agg — see cross-sec-no-universe-pin below). Re-collect required after fix. |
| P0 | cross-sec-no-universe-pin | **NEW 2026-06-15 — `cross_sectional_rank` ranks over the RAW trade tape, NOT the universe.** `compute()`/`compute_latest()` (cross_sectional_rank.py:67,92) read `minute_agg` and only pin to a universe IF `"universe" in ctx.frames` — which is NOT wired in the live path (evidence: 9,913 distinct symbols/day, nsym up to 4,809/min in the stored output, far exceeding any equity universe; 32.8% funds present). So even with a CLEAN universe table the ranks would still be polluted, because the group never consults it. The parity NOTE at the top of the file (FEATURE_TAXONOMY gap #3) explicitly flags this as the standing follow-up — it is now BITING. Cross-section is also unstable minute-to-minute (nsym 63→4,809) so a name's percentile depends on how many OTHER names happened to print that minute = noisy + non-reproducible live↔backfill. FIX: wire the (clean) per-minute universe frame into ctx.frames for this group so the `if "universe" in ctx.frames` branch activates, and gate output to universe members only. | OPEN — routes to modeller (owns group) + prod (provides universe frame to feature-computer). Blocks any cross-sectional model use of these ranks. |
