...
| `1-58/3 * * * *` | `ops/collect_store_glimpse.py` | **DEPRECATED — REMOVE ON DEPLOY of the `store-glimpse-worker` compose service.** Precomputed the OLD group×date `/store-glimpse` grid into Redis on a host cron. Superseded by the always-warm `store-glimpse-worker` container (a permanent rebuild LOOP, `restart: unless-stopped`), which builds the NEW ticker×date coverage matrix continuously so a cold/expired cache can never surface the "warming…" placeholder. The old group×date glimpse + its cron remain only until PR 2 deletes the old page; once `store-glimpse-worker` is up, delete this crontab line (`crontab -e`). | benign (READ-ONLY w.r.t. the store; idempotent) | `~/.quant-ops/collect_store_glimpse.log` |
