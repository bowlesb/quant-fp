...
| `1-58/3 * * * *` | `ops/collect_store_glimpse.py` | every 3 min precompute the `/store-glimpse` grid + per-group ticker drills into the persistent (Redis) cache so the page serves sub-ms instead of paying the ~50s on-request build. Execs `python -m store_glimpse_cache` INSIDE `quant-dashboard-1` (the container has quantlib/polars + the read-only `/store` + a route to `quant-redis`) — same docker-exec pattern `healthcheck.sh` uses into `feature-computer`. | benign (READ-ONLY w.r.t. the store: `/store` is mounted read-only; idempotent — each run overwrites the same Redis blobs; ~105s/run < the 3-min interval) | `~/.quant-ops/collect_store_glimpse.log` |
