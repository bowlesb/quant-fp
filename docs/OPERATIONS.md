...
| `45 14 * * *` | `ops/trust_random_check.sh` | DAILY RANDOM re-check of TRUSTED features on a random recent clean day; un-trusts clean-day failures (docs/TRUST_REDESIGN.md). Daily (not weekly) so trust regressions surface fast during active development; skips unsettled days (no false un-trust) | conservative (only un-trusts on a positive clean-day disagreement; re-runs an idempotent sweep) | `~/.quant-validation/trust_random_check.log` |
...
| `*/15 * * * *` | `ops/data_freshness.sh` | **Non-tape data-freshness alert** (SEC EDGAR filings + Alpaca news). Reads the TRUE newest ingest instant per source — max `discovered_at` for live `source='stream'` filings + max `live_arrival` `available_at` in `/store/news` — and WARN/STALEs when ingest has stalled beyond a market-hours-aware threshold (EDGAR WARN 45m / STALE 120m, news WARN 90m / STALE 240m, applied ONLY Mon-Fri ~06:00-22:00 ET; weekend/overnight lulls report INACTIVE, never alert). Closes the #311 blind spot where `quant-edgar-1`'s `poll: 100 filings upserted` log can't distinguish a real stall from health (it reports parsed feed size, not rows inserted). Execs the probe inside `quant-dashboard-1` (has quantlib+polars+psycopg+DB env+`/store`), exactly like `healthcheck.sh`/`collect_store_glimpse.py`. | benign — READ-ONLY (one DB SELECT + a polars scan of the newest 4 news partitions); NEVER touches the live ingesters (`quant-edgar-1`/`news-capture`); off-hours self-suppresses. Exit 1 only when a source is STALE during business hours. | `~/.quant-ops/data_freshness.jsonl` |
