...
|-----|------|---------------|--------------------|
| P1 | FP feature compute does NOT hit per-minute latency target single-process | mem_bench (2026-06-13, 502 minute-path feats / 26 groups): 1k tickers x120m buffer = 2.8s, 2.5k = 9.1s (~linear) → single-process 10k ≈ tens of seconds; CORRECTNESS needs a ~300m buffer (240m windows) making it ~2.5× heavier. Aspirational <2s is blown; even <60s has no single-process margin. | Shard by symbol across the worker pool (ingestor already shards): 10k/N shards on 32 threads → per-shard 625-1250 names ≈ 2-9s in parallel, feasible within the minute. Standing practice (Ben, 6/13): re-run mem_bench at 10k EVERY feature batch; measure REAL streaming latency against live Alpaca next session. Further levers: drop very-long windows, latest-minute compute mode. |
