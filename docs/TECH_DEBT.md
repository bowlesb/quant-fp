...
|-----|------|---------------|--------------------|
| P1 | FP feature compute exceeds the per-minute budget single-process (MEASURED + design done) | mem_bench (2026-06-13, 502 feats / 26 groups, 32-thread box): single-process 10k×120m = 47.5s; 10k×300m (correctness buffer) = **140s / 25GB** — 2.3× over the 60s minute. Per-group profiler: OLS-kernel groups (price_volume/trend_quality/return_dynamics/distribution/market_beta) dominate at 10-30× µs/feature. | NOT pruning — designed in docs/SCALABILITY.md. KEY measured result: **symbol-sharded multiprocessing is 3.4× faster** than single-process threading (32 procs×1thr 10k×120m = ~14s vs 47.5s; polars .over() threads poorly) → 300m projects to ~41s, under budget, BEFORE the latest-minute optimization (removes ~300× whole-buffer waste). Build order: (1) live per-group timing + latency API + CI gate, (2) OLS-kernel/sort reuse [2-5×, byte-identical], (3) symbol-sharded persistent process pool + cross-sectional gather phase + partition-disjoint DB COPY, (4) latest-minute emission behind a parity test, (5) re-measure & only then weigh a Rust polars-plugin for any remaining hot group. Standing rule (Ben): profile every feature batch; both timing + process-split are first-class. (Earlier note "sharding won't help" was WRONG — confounded by thread-division.) |
