...
| `GET http://<host>:8088/api/lifecycle-state` | CERTIFICATION-LIFECYCLE state: per feature-group its furthest stage `UNVERIFIED → MONITORING → CERTIFIED → TRUSTED`, read off `within_day_assignment` (who owns the monitoring lock) + `within_day_parity_cert` (latest within-day intraday verdict: stable_cycles/value_rate/cert_day) + `feature_trust` (the permanent binary grant). The **Lifecycle** SPA tab renders it. See `docs/WITHIN_DAY_PARITY_CERTIFICATION.md` |
| `GET http://<host>:8088/api/lifecycle-trend` | The lifecycle **trend** behind the snapshot — per calendar day: within-day cert activity (`within_day_parity_cert` by `cert_day`), trust grants (`feature_trust.trusted_day`), un-trust events, and the running cumulative trusted line. The **Lifecycle** tab renders it as a compact "Trust over time" strip. `trusted_now` (live TRUSTED count) is surfaced alongside the cumulative line because grants predating the `trusted_day` column do not appear on it. Three small indexed GROUP BYs, 120s server cache |
