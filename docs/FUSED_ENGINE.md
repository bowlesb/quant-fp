...
## Expected payoff
Single-process per-shard compute ~300ms → ~100ms (one marshal instead of ~12), and lower memory traffic
should also shrink the concurrency gap — targeting the full 10k vector well under ~300ms p99, with a path
toward ~100ms.
