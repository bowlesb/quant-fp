...
| [`docs/FEATURE_EFFICIENCY_AUDIT.md`](FEATURE_EFFICIENCY_AUDIT.md) | per-group state-holding audit (A cache-once / B O(1)-fold / Rust-resident) vs current realization + the ranked GAP list + held-state migration gates (#334) | humans; where a slow group's optimal-state gap is written up |
...
- `kind`, `mechanism`, `incremental_ready` (`ready` / `parked` / `n-a` / `stateful-gated`) — the stable framing (above).
- `p50_ms`, `p95_ms`, `p99_ms` — the measured typical / upper / tail per-group `compute_latest` cost.
- `note` (OPTIONAL — present only on a few groups, e.g. `swing`'s FP_SWING_STATEFUL gate) — a per-group caveat; render in the hover when present.
