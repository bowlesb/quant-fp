"""Postgres-backed bet ledger for the reversion strategy — its OWN ``strat_reversion`` schema.

Structurally identical to ``strategies/smoke/bet_store.BetStore`` (same notional-sized, status-partitioned
``bets`` table, same idempotent open/fill/close lifecycle) — the difference is ONLY the strategy name, so
``StrategyStore`` derives the isolated ``strat_reversion`` schema. Keeping a thin per-strategy store (rather
...
class BetStore:
    """Durable OPEN/CLOSED bet ledger in the ``strat_reversion`` Postgres schema (via ``StrategyStore``)."""
    ...
    def has_open_symbol(self, symbol: str) -> bool:
        """Whether this symbol already has an un-closed bet — reversion holds at most one bet per name
        so it never stacks multiple longs on the same stretched symbol."""
        rows = self._store.query(
