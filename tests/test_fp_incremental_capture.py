...
from quantlib.features.reduction_anchor import SESSION_MINUTES, attach_volume_anchor
...
INCREMENTAL_UNSAFE = {
    ...
    "residual_analysis",
    # The 5 REAL-DATA soak NO-GO breachers (docs/INCREMENTAL_READINESS.md, 2026-06-17 A/B): rare guard-straddle
    # / power-sum-cancellation cells the synthetic degenerate stream cannot reach on real gappy tape. Re-gated
    # to batch until the cancellation-free reduction-denom fix lands. Same engineering class as the trio above.
...
# The 5 groups the real-data soak (docs/INCREMENTAL_READINESS.md) re-gated to batch — they must be
# incremental_safe=False and therefore byte-identical to batch under FP_INCREMENTAL. (An earlier synthetic-only
# pass had flipped trend_quality + clean_momentum to safe; the real-data soak reverted that.)
INCREMENTAL_REGATED = {
    "range_expansion",
    "trend_quality",
    "clean_momentum",
...
def test_regated_groups_are_incremental_unsafe() -> None:
    """The 5 real-data-soak NO-GO groups carry ``incremental_safe = False`` (re-gated to batch), so they ride
    the batch path under FP_INCREMENTAL — proven byte-identical to batch by
...
def _with_volume_anchor(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    divides by the session-minute count to reach the per-minute scale (reduction_anchor._SESSION_MINUTES); so
    here we synthesize a daily snapshot at DAILY-TOTAL scale (the per-symbol per-minute level x the session
    ...
    daily = (
        ...
        .agg((pl.col("volume").last() * SESSION_MINUTES).alias("volume"))
...
def test_gappy_denom_group_now_clean_after_p2_neumaier(group_name: str) -> None:
    """⭐ P2 PROOF (stable summation): on THIS gappy near-constant-return SYNTHETIC walk these corr-family
    groups USED to breach engine-vs-batch. The Neumaier-compensated running sum (``_comp`` carries the
    add/expire rounding loss) now makes the corr-kernel power-sum denominator match the batch fresh sum, so
    engine-vs-batch is CLEAN on this walk — the synthetic breach is CLOSED. This is NECESSARY but NOT
    SUFFICIENT for arming: the real-data A/B soak (docs/INCREMENTAL_READINESS.md) is the arming authority, and
    it found a rarer autocorr null-flip cell on real gappy tape this synthetic walk does not reproduce — so
    return_dynamics is re-gated ``incremental_safe=False`` (see INCREMENTAL_UNSAFE). This test pins the
    synthetic-walk parity (the P2 regression guard); it does not by itself unblock the flip."""
