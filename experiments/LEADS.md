...
|---|---|---|---|---|
| 2026-06-16 | **★★★ W11 OVERNIGHT-BETA premium — CERTIFIED net-of-(modeled)-cost OOS. The program's ONLY surviving edge (6 KILLs around it).** | FULL-POWER certify on **top-300 liquid, 378d/18mo, ~60 names/quintile, 15 rebalances** (`2026-06-16-w11-overnight-beta/certify300*`). High-minus-low-beta L/S **+41.6 bps/day OVERNIGHT (87% of rebalances +) vs −3.9 INTRADAY**, full-sample bootstrap CI [+19.6,+63.6] excludes 0; canary clean (beta-permute → +3, straddles). **Confound control at 2× power: removing all 27 crypto/AI gappers moves it only +41.6→+36.0 (CI [+17.9,+53.5] still excludes 0) → durable beta-RISK premium, NOT the gap regime.** Director-verified vs certify300_results.json. | **THE GATE CLEARED:** OOS net INCLUDING the 3bps spread + 5bps MOO/MOC auction stress now **EXCLUDES zero in BOTH universes** — FULL **+28.4 [+1.2,+53.3]**, SPEC-EXCL **+30.5 [+7.9,+48.9]** (the 168-name run straddled; the wider cross-section tightened + lifted it off zero). Turnover ~17-20% (low). | **PROMOTE to a PAPER container** (PR #64 design, awaiting Lead review) — now to MEASURE that the +28-30 bps overnight net survives REAL auction fills vs the 5bps MODEL (the only remaining unknown), not as a pass/fail gate. Residual caveat: **2025-H1 (bull half) shows NO split** — regime-conditional; 2025-H2 (+54/−17) + 2026-H1 clean. Next: paper-validate real slippage, then certify on 3-5yr bars. |
