...
`tests/test_fp_lookahead.py::test_no_lookahead_bar_features` was FAILING on origin/main when this feature
was first built — the differing cells were `residual_analysis`'s `residual_skew_{5,10,15,20,30,60}m`
(a look-ahead leak), NOT caused by this work (my group self-excludes when its inputs are absent). The Lead
dispatched a focused fix that landed in **#30** (`feat(parity): ... fix residual_skew live==backfill`), so
this branch is rebased onto a base where that test passes again. Flag resolved.
