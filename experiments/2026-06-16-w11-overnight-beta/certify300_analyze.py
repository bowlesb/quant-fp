"""W11 FULL300 CERTIFICATION — Overnight-BETA premium on the 378d (18mo) deep history, top-300 universe.

REUSES the EXACT W11 certify construction (certify_analyze.py): 60d rolling SPY beta, monthly
beta-quintile high-minus-low overnight/intraday/24h L/S, speculation-cohort exclusion, sub-period
split, spread + MOO/MOC auction-slippage stress, per-rebalance bootstrap CI, walk-forward OOS,
beta-permute canary. ONLY the universe changes: top-300 by median dollar-volume (now all at 378d),
giving ~60 names/quintile (vs ~33 in the preliminary 168-name certify).

Writes certify300_results.json (does NOT touch the original certify_results.json).
"""
...
DAILY = "/app/experiments/2026-06-16-w11-overnight-beta/certify300_daily.parquet"
OUT_JSON = "/app/experiments/2026-06-16-w11-overnight-beta/certify300_results.json"

N_LIQUID = 300       # FULL POWER: top-300 liquid single stocks → ~60/quintile with 5 quintiles
...
def load_returns(exclude_speculation: bool) -> tuple[pl.DataFrame, pl.DataFrame, list, list[str]]:
    ...
    return tradeable, mkt, dates, liquid_syms
...
def run(exclude_speculation: bool = False, permute_beta: bool = False) -> tuple[list[dict], int, list[str]]:
    daily, mkt, dates, liquid_syms = load_returns(exclude_speculation)
    ...
    for r, est_idx in enumerate(rebal_idxs):
        ...
        results.append(ls)
    return results, n_universe, liquid_syms
...
def analyze(label: str, exclude_speculation: bool) -> dict:
    real, n_universe, liquid_syms = run(exclude_speculation=exclude_speculation, permute_beta=False)
    canary, _, _ = run(exclude_speculation=exclude_speculation, permute_beta=True)
    tos = turnover(real)
    ...
    spec_in_univ = sorted(set(liquid_syms) & SPECULATION_COHORT)

    return {
        "label": label,
        "exclude_speculation": exclude_speculation,
        "n_universe": n_universe,
        "n_speculation_in_universe": len(spec_in_univ),
        "speculation_in_universe": spec_in_univ,
        "n_rebalances": n,
...
def main() -> None:
    ...
    for blk in (full, no_spec):
        print(f"\n=== {blk['label']} ===")
        print(f"n_universe={blk['n_universe']}  n_spec_in_univ={blk['n_speculation_in_universe']}  n_rebalances={blk['n_rebalances']}  q_size={blk['q_size']}  avg_turnover={blk['avg_turnover']:.3f}")
