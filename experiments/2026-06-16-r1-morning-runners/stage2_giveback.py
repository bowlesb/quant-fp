...
def main() -> None:
    ...
    for i, arg in enumerate(args):
        result = _build_one(arg)
        if result is not None:
            rows.append(result)
            dates.append(arg[1])
        if (i + 1) % 200 == 0:
            print(f"  built {i + 1}/{len(args)}", flush=True)
    ...
    # Honest test: a no-skill model gives OOS R2 ~ 0 (NOT the shuffled-label canary's wildly
    # negative R2 — beating that proves nothing). Magnitude is forecastable only if OOS R2 is
    # meaningfully positive. Direction (hard-fade) is judged by AUC lift over 0.5.
    mag_forecastable = oos_r2 > 0.02
    dir_signal = oos_auc > 0.58
    print(f"\nMAGNITUDE: OOS R2 {oos_r2:+.4f} -> {'forecastable' if mag_forecastable else 'NOT forecastable (R2~0)'}")
    print(f"DIRECTION: hard-fade AUC {oos_auc:.3f} -> {'mild signal' if dir_signal else 'no signal'}")
