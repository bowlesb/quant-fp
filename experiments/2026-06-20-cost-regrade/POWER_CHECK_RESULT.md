...
would be the post-hoc fishing the eligibility filter exists to prevent).

## The counts (42 dates, pool = top-400 liquid by ADV; realized half-spread from Stage-1 measured cost)
| universe | names/date (median) | 2%-cut/side | 5%-cut/side | 10%-cut/side |
|---|---:|---:|---:|---:|
| **<2.0bps** (pre-reg primary) | **20** (13-34) | 1 | 1 | 2 |
| **<3.0bps** (pre-committed fallback) | **43** (30-69) | 1 | 2 | 4 |
| liquid-200 (full, the G0a universe) | ~190 | 4 | 10 | 20 |

A {2,5,10}%-cut cross-sectional L/S needs enough names per side for a stable basket. At <2.0bps (1-2/side)
and even <3.0bps (1-4/side) the threshold curve the gate requires is not meaningfully estimable — a 2%-cut
"basket" of 1 name/side is a single-name bet, not a cross-section.

## Why the cheap universe is so thin (the structural reason — characterized before any $-result)
Realized half-spread in the liquid-200 (n=1909 over 10 dates): p10=1.85, median=**7.45**, p90=19.47 bps.
- Only **22%** of liquid-200 names are CHEAPER than the old 3.0bps stub (the overcharged set the re-grade
  could revive).
- **78%** are MORE expensive than the stub (median 7.45 vs 3.0) — accurate cost makes these MORE-null, not
...
I will NOT silently relax to a third boundary (post-hoc universe selection is the exact fishing the filter
forbids). Two principled paths, for the Lead's gate-read:

- **Option A — re-grade the FULL liquid-200 (the original G0a universe) under accurate PER-NAME cost.** This
