...
# up/down fraction of the whole market (+ each sector); sector_return/sector_beta aggregate each GICS
# sector's mean return over every symbol in it. Run per-shard they would see only ~1/8 of the universe
# and produce 8 different "sector-wide" values per minute, breaking live↔backfill parity — so they MUST
# run once in the reader's gather phase over every symbol.
REDUCE_GROUPS: tuple[str, ...] = ("cross_sectional_rank", "breadth", "sector_return", "sector_beta")
