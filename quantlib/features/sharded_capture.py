...
# realized vol over the whole universe. Run per-shard they would see only ~1/8 of the universe and produce
# 8 different "market-wide" values per minute, breaking live↔backfill parity — so they MUST run once in the
# reader's gather phase over every symbol.
REDUCE_GROUPS: tuple[str, ...] = ("cross_sectional_rank", "breadth", "market_turbulence")
