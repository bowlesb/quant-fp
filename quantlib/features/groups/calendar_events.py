...
class CalendarEventsGroup(FeatureGroup):
    ...
    def declare(self) -> list[FeatureSpec]:
        return [
            FeatureSpec(name="day_of_month_norm", description="Calendar day of month in ET divided by 31 (position through the month, 0-1).",
                        dtype="Float64", valid_range=(0.0, 1.04), nan_policy="none", layer="A"),
            FeatureSpec(name="week_of_month", description="Week of the month in ET (1-5), as ceil(day_of_month / 7).",
                        dtype="Float64", valid_range=(1.0, 5.0), nan_policy="none", layer="A"),
            FeatureSpec(name="is_opex_day", description="1.0 when the bar is on monthly options-expiration Friday (the third Friday of the month) in ET, else 0.0.",
                        dtype="Float64", valid_range=(-0.01, 1.01), nan_policy="none", layer="A"),
            FeatureSpec(name="is_triple_witching", description="1.0 on quarterly triple-witching (third Friday of March/June/September/December) in ET, else 0.0.",
                        dtype="Float64", valid_range=(-0.01, 1.01), nan_policy="none", layer="A"),
            FeatureSpec(name="is_quarter_end_month", description="1.0 when the bar falls in a quarter-end month (Mar/Jun/Sep/Dec) in ET, else 0.0.",
                        dtype="Float64", valid_range=(-0.01, 1.01), nan_policy="none", layer="A"),
            FeatureSpec(name="is_first_week", description="1.0 when the ET calendar day of month is 7 or earlier (first week), else 0.0.",
                        dtype="Float64", valid_range=(-0.01, 1.01), nan_policy="none", layer="A"),
            FeatureSpec(name="is_last_week", description="1.0 when the ET calendar day of month is 22 or later (last week, month-end window), else 0.0.",
                        dtype="Float64", valid_range=(-0.01, 1.01), nan_policy="none", layer="A"),
        ]
