...
class MomentumRunGroup(FeatureGroup):
    ...
    def compute_latest(self, ctx: BatchContext) -> pl.DataFrame:
        """Window-sliced live path: the SAME ``compute()`` on the trailing ``LOOKBACK_MINUTES`` it reads,
        filtered to T. Parity-true by construction — the run-length is capped to in-window positions and the
        residual_skew fit is window-local, so bars dropped before the slice cannot change T."""
