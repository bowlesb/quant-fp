...
class SignedTradeRatioGroup(ReductionGroup):
    ...
    def reduced(self) -> dict[str, tuple[pl.Expr, tuple[str, ...], tuple[int, ...]]]:
        # Group-unique reduced keys (``str_*``): the unified-emit canonical column names are ``__{stat}_{key}_{w}``
        # over a SINGLE namespace shared across groups, so a key reused by another group (``trade_flow`` already
        # reduces ``signed_volume`` and ``price_volume`` already reduces ``volume``) would duplicate a column in
        # that shared schema. Prefixing keeps this group's sums distinct.
        return {
            "str_sv": (pl.col("signed_volume"), ("sum",), WINDOWS),
            "str_vol": (pl.col("volume"), ("sum",), WINDOWS),
        }
    ...
    def assemble(self) -> dict[str, pl.Expr]:
        ...
        return {
            f"signed_trade_ratio_{w}m": pl.when(sum_("str_vol", w) > 0)
            .then(sum_("str_sv", w) / sum_("str_vol", w))
            .otherwise(None)
            for w in WINDOWS
        }
