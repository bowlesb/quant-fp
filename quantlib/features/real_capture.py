...
from quantlib.features.loaders import load_filings, load_news_features, load_reference, load_universe
...
def run_capture(symbols: list[str], root: str, mode: str, window: int = DEFAULT_BUFFER_MINUTES, day: str | None = None) -> None:
    ...
    if day is not None:
        ...
        # News session snapshot (article tape with baseline sentiment, 9d lookback); same per-minute
        # available_at<=minute gate inside the news_sentiment group → point-in-time, backfill==live.
        snapshots["news"] = load_news_features(day)
...
def run_sharded_capture(  # pragma: no cover (live multiprocess loop; logic is unit-tested in sharded_capture)
    ...
    if snapshots is None:
        ...
        if day is not None:
            ...
            # available_at<=minute gate inside the edgar group makes it point-in-time → backfill==live.
            snapshots["filings"] = load_filings(day)
            # News session snapshot (article tape + baseline sentiment, 9d lookback); same per-minute
            # available_at<=minute gate inside the news_sentiment group → point-in-time, backfill==live.
            snapshots["news"] = load_news_features(day)
