...
REL_TOL = 1e-6  # a cell "matches" when essentially identical modulo float noise
...
def parity_test(
    ...
    for feature in feature_cols:
        ...
        matched = both & ((live_col - back_col).abs() <= REL_TOL * (1.0 + back_col.abs()))
        for tier in (1, 2, 3):
...
def main() -> None:
    ...
    print(f"=== T+1 Settled-Day Parity — {day} (live=stream vs backfill, tol={REL_TOL}) ===")
    print(report)
